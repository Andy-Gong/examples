...
public class RedLockExample {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        final RedissonRedLock redLock = (RedissonRedLock) redisson.getRedLock(lock1, lock2, lock3);
        Thread t = new Thread(new Runnable() {
            public void run() {
                try {
                    boolean hasAcquiredLock = redLock.tryLock(1000, 10000, TimeUnit.MILLISECONDS);
                    log.info("lock successfully t1, hasAcquiredLock: " + hasAcquiredLock);
                    if (!hasAcquiredLock) {
                        Thread.sleep(10000);
                        hasAcquiredLock = redLock.tryLock(1000, 10000, TimeUnit.MILLISECONDS);
